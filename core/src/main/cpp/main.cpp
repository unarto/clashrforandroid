...
Master::Master(JavaVM *vm, JNIEnv *env) : vm(vm) {
  ...
  cClashException = g<jclass>(
      env, env->FindClass("com/github/kr328/clash/core/bridge/ClashException"));
  cTraffic = g<jclass>(
      env, env->FindClass("com/github/kr328/clash/core/model/Traffic"));
  cGeneral = g<jclass>(
      env, env->FindClass("com/github/kr328/clash/core/model/General"));
  cCompletableFuture =
      g<jclass>(env, env->FindClass("java/util/concurrent/CompletableFuture"));
  cProxyGroup = g<jclass>(
      env, env->FindClass("com/github/kr328/clash/core/model/ProxyGroup"));
  cProxy =
      g<jclass>(env, env->FindClass("com/github/kr328/clash/core/model/Proxy"));
  cLogEvent = g<jclass>(
      env, env->FindClass("com/github/kr328/clash/core/event/LogEvent"));
  iTunCallback = g<jclass>(
      env, env->FindClass("com/github/kr328/clash/core/bridge/TunCallback"));
  iLogCallback = g<jclass>(
      env, env->FindClass("com/github/kr328/clash/core/bridge/LogCallback"));
  cClashExceptionConstructor =
      env->GetMethodID(cClashException, "<init>", "(Ljava/lang/String;)V");
  cTrafficConstructor = env->GetMethodID(cTraffic, "<init>", "(JJ)V");
  cGeneralConstructor =
      env->GetMethodID(cGeneral, "<init>", "(Ljava/lang/String;IIII)V");
  cCompletableFutureConstructor =
      env->GetMethodID(cCompletableFuture, "<init>", "()V");
  cProxyGroupConstructor =
      env->GetMethodID(cProxyGroup, "<init>",
                       "(Ljava/lang/String;Ljava/lang/String;Ljava/lang/"
                       "String;[Lcom/github/kr328/clash/core/model/Proxy;)V");
  cProxyConstructor = env->GetMethodID(
      cProxy, "<init>", "(Ljava/lang/String;Ljava/lang/String;J)V");
  cLogEventConstructor =
      env->GetMethodID(cLogEvent, "<init>", "(Ljava/lang/String;)V");
  mCompletableFutureComplete =
      env->GetMethodID(cCompletableFuture, "complete", "(Ljava/lang/Object;)Z");
  mCompletableFutureCompleteExceptionally = env->GetMethodID(
      cCompletableFuture, "completeExceptionally", "(Ljava/lang/Throwable;)Z");
  mTunCallbackOnNewSocket =
      env->GetMethodID(iTunCallback, "onNewSocket", "(I)V");
  mTunCallbackOnStop = env->GetMethodID(iTunCallback, "onStop", "()V");
  mLogCallbackOnMessage =
      env->GetMethodID(iLogCallback, "onMessage",
                       "(Lcom/github/kr328/clash/core/event/LogEvent;)V");
...
Master::Context::Context(JNIEnv *env) { this->env = env; }

jthrowable Master::Context::newClashException(const char *reason) {
  return reinterpret_cast<jthrowable>(env->NewObject(
      master->cClashException, master->cClashExceptionConstructor,
...
jobject Master::Context::newCompletableFuture() {
  return env->NewObject(master->cCompletableFuture,
                        master->cCompletableFutureConstructor);
}
...
bool Master::Context::completeCompletableFuture(jobject completable,
                                                jobject object) {
  return env->CallBooleanMethod(completable, master->mCompletableFutureComplete,
                                object);
}

bool Master::Context::completeExceptionallyCompletableFuture(
    jobject completable, jthrowable throwable) {
  return env->CallBooleanMethod(
      completable, master->mCompletableFutureCompleteExceptionally, throwable);
}

const_buffer_t Master::Context::createConstBufferFromByteArray(
    jbyteArray array) {
  return {.buffer = env->GetByteArrayElements(array, nullptr),
          .length = env->GetArrayLength(array)};
}

void Master::Context::releaseConstBufferFromByteArray(jbyteArray array,
                                                      const_buffer_t &buffer) {
  env->ReleaseByteArrayElements(
      array,
      const_cast<jbyte *>(reinterpret_cast<const jbyte *>(buffer.buffer)),
      JNI_ABORT);
}
...
jobjectArray Master::Context::createProxyGroupArray(int size,
                                                    jobject elements[]) {
...
jobject Master::Context::createProxy(char const *name, proxy_type_t type,
  ...
  return env->NewObject(master->cProxy, master->cProxyConstructor,
                        env->NewStringUTF(name), ts, delay);
}

jobject Master::Context::createProxyGroup(char const *name, proxy_type_t type,
                                          char const *current,
                                          jobjectArray proxies) {
  jstring ts = nullptr;
  ...
  return env->NewObject(master->cProxyGroup, master->cProxyGroupConstructor,
                        env->NewStringUTF(name), ts, env->NewStringUTF(current),
                        proxies);
}

void Master::Context::logCallbackMessage(jobject callback, const char *data) {
  jobject event = env->NewObject(
      master->cLogEvent, master->cLogEventConstructor, env->NewStringUTF(data));
