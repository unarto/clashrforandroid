...
class Master {
 ...
 public:
  ...
  template <class R>
  static R runWithAttached(const std::function<R(JNIEnv *)> &func);
...
class Master::Context {
 ...
 public:
  ...
  jobject newGeneral(char const *mode, jint http, jint socks, jint redirect,
                     jint mixed);
 ...
 public:
  ...
  bool completeExceptionallyCompletableFuture(jobject completable,
 ...
 public:
  ...
  jobject createProxyGroup(char const *name, proxy_type_t type,
  ...
  void releaseConstBufferFromByteArray(jbyteArray array,
...
template <class R>
R Master::runWithContext(JNIEnv *env,
                         const std::function<R(Master::Context *)> &func) {
...
template <class R>
R Master::runWithAttached(const std::function<R(JNIEnv *)> &func) {
